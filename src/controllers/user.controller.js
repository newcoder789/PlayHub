...
const logoutUser = asyncHandler( async (req,res)=>{
    await User.findByIdAndUpdate(
        req.user._id,
        {
            // this does not send data because it do not remove field from document 
            // $s et:{
            //     refreshToken: undefined ( null will work here )
            // }
            $unset:{
                refreshToken:1
            }
...
const updateUserAvatar = asyncHandler( async(req,res)=>{
    ...
    // Get the user to retrieve the old avatar
    const user = await User.findById(req.user?._id);
    ...
    // Check if the user has an old avatar
    const oldAvatarPublicId = user.avatar?.split('/').pop().split('.')[0]; // Extract the public ID from the avatar URL
    ...
    if (oldAvatarPublicId) {
        await cloudinary.uploader.destroy(oldAvatarPublicId, (err, result) => {
            if (err) {
    ...
    .status(200)
    .json(
        new ApiResponse(200, changedUser, "Avatar image updated successfully")
    )
})
...
const updateUserCoverImage = asyncHandler( async(req,res)=>{
    ...
    if (oldCoverImagePublicId) {
        await cloudinary.uploader.destroy(oldCoverImagePublicId, (err, result) => {
            if (err) {
                console.error("Error deleting old avatar:", err);
            } else {
                console.log("Old avatar deleted:", result);
    ...
    .json(
        new ApiResponse(
            200, updatedUser, "coverImage updated successfully."
        )
    )
